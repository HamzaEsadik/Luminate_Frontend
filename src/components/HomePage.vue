...
<div v-if="this.inv > 0" class="px-4 sm:px-8 w-full h-10 bg-green-400 rounded-xl flex flex-row justify-between items-center">
  <h5 class="text-sm sm:text-base truncate">{{ $store.state.invitations.data[0].company }} invited you to join their team</h5>
...
<div class="w-full grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2">
  <!--projects-->
  <div class="bg-secondary-light rounded-xl px-4 sm:px-8 py-8 sm:py-14 flex flex-col">
    <div class="h-16 sm:h-20 flex flex-row justify-between items-center">
      <h3 class="text-lg sm:text-xl">Projects</h3>
        <button v-if="$store.state.user_ismanager" class="text-xs sm:text-sm w-32 sm:w-44 font-medium" @click="this.$store.commit('open_project')">Add new project</button>
    </div>
    <ul class="grid grid-cols-1 gap-4 sm:gap-8">
      <label @change="tasks" v-for="(project, index) in $store.state.projects.data" :key="index" class="has-[:checked]:border-primary flex flex-col gap-3 sm:gap-5 border-2 p-3 sm:p-4 rounded-lg border-gray-300 cursor-pointer after:content-[''] after:ml-0.5 text-center">
        ...
        <div class="flex flex-row gap-2 sm:gap-3">
          <div class="h-5 w-full">
            <h5 class="line-clamp-1 text-sm sm:text-base font-semibold">{{ project.title }}</h5>
          </div>
        ...
        <div>
          <p class="font-medium text-left text-sm sm:text-base">{{ project.description }}</p>
        </div>
      </label>
    </ul>
  </div>
  ...
  <!--meetings-->
  <div class="bg-secondary-light rounded-xl px-4 sm:px-8 py-8 sm:py-14 flex flex-col cursor-pointer">
    <div class="h-16 sm:h-20 flex flex-row justify-between items-center">
      <h3 class="text-lg sm:text-xl">Upcoming meetings</h3>
        <button v-if="$store.state.user_ismanager" class="text-xs sm:text-sm w-32 sm:w-44 font-medium" @click="this.$store.commit('open_meeting')">Add new meeting</button>
    </div>
    <ul class="grid grid-cols-1 gap-4 sm:gap-8">
      <li v-for="(meeting, index) in $store.state.meetings.data" :key="index" class="flex flex-row gap-2 sm:gap-3 items-center">
        <div class="h-12 w-12 sm:h-14 sm:w-14 rounded-xl bg-white shrink-0 flex flex-col">
          ...
          <div class="w-full h-full rounded-b-xl flex flex-col justify-center">
            <p class="text-xl sm:text-2xl font-bold text-primary h-fit">25</p>
          </div>
        </div>
        <div class="h-14 w-full">
          <h5 class="line-clamp-1 text-sm sm:text-base">{{ meeting.title }}</h5>
          <h5 class="text-xs sm:text-sm text-slate-500">{{ meeting.datetime }}</h5>
        </div>
  ...
  <div class="bg-secondary-light rounded-xl px-4 sm:px-8 py-8 sm:py-14 flex flex-col">
    <div class="h-16 sm:h-20 flex flex-row justify-between items-center">
      <h3 class="text-lg sm:text-xl">Tasks</h3>
        <button v-if="$store.state.user_ismanager" class="text-xs sm:text-sm w-32 sm:w-44 font-medium" @click="this.$store.commit('open_task')">Add new task</button>
    </div>
    <select @change="tasks" v-model="selected_user" v-if="$store.state.user_ismanager" name="user" id="user" class="h-12 sm:h-14 text-sm sm:text-lg bg-secondary-light border-gray-300 active:border-gray-300">
      <option v-for="user in $store.state.team" :key="user.id" :value="user.id">{{ user.first_name + ' ' + user.last_name}}</option>
    </select>
    <ul class="grid grid-cols-1 gap-4 sm:gap-8">
      <li v-for="task in $store.state.tasks.data" :key="task.id" class="flex flex-col gap-3 sm:gap-5 border-2 p-3 sm:p-4 rounded-lg border-gray-300 has-[:checked]:border-primary">
        <div class="flex flex-row gap-2 sm:gap-3">
          <div class="h-5 w-full">
            <h5 class="line-clamp-1 text-sm sm:text-base font-semibold">{{ task.title }}</h5>
          </div>
        ...
        <div>
          <p class="font-medium text-left text-sm sm:text-base">{{ task.description }}</p>
        </div>
