<template>
  <div class="fixed w-full h-screen z-30 bg-black/85 flex justify-center items-center">
    <div class="w-[40rem] h-fill bg-secondary-light z-20 rounded-xl flex flex-col p-8 gap-6 relative">
      <svg @click="this.$store.commit('close_task')" class="absolute right-6 z-30 w-5 h-5 stroke-slate-500 cursor-pointer stroke-2" data-slot="icon" data-darkreader-inline-stroke="" fill="none" stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      ...
      <form action="#" @submit.prevent="sub">
        ...
        <input type="date" v-model="task.start_at" required>
        <label for="ends_at">Ends at</label>
        <input type="date" v-model="task.ends_at" required>
...
export default {
  ...
  methods: {
    sub() {
      ...
      axios.post(`${process.env.VUE_APP_API_BASE_URL}/tasks`,this.task, {
        headers: headers,
      })
        .then(response => {
          location.reload();
